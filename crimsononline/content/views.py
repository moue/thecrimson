...
def writer(request, pk, f_name, m_name, l_name, page=1, sections=None, types=None):
    ...
    w.sports_number_of_articles = Article.objects.filter(contributors=w and section='Sports' and not tag='Sports Blog').count()
    w.sportsblog_number_of_articles = Article.objects.filter(contributors=w and tag='Sports Blog').count()
