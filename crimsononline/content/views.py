...
def writer(request, pk, f_name, m_name, l_name, page=1, sections=None, types=None):
    ...
    w.sports_number_of_articles = Article.objects.filter(contributors=w).filter(section='Sports').exclude(tags='Sports Blog').count()
    w.sportsblog_number_of_articles = Article.objects.filter(contributors=w).filter(tags='Sports Blog').count()
...
def section_sports(request):
    ...
    ** There's tons of crap on this page: **
    ...
    stories = top_articles(section).filter(Q(group__isnull=True) | (Q(group__isnull=False) & ~Q(group=sportsblog)))
    rotate = rotatables(section)
...
def commencement2010(request):
    ...
    galleries = Gallery.objects.prioritized(30).filter(tags=ctag)[:6]
